...
DATA_ZIP_FILE = "base_queimadas_INPE_2020_2025_didatica_limpa.zip"
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        ...
        help=(
            ...
            "Para aceitar arquivos de até 500 MB, mantenha o arquivo .streamlit/config.toml incluído no projeto."
        ),
    )
    st.caption("Limite configurado para upload: até 500 MB. O ZIP deve conter arquivo(s) CSV.")
...
try:
    ...
    elif os.path.exists(DATA_ZIP_FILE):
        df = load_data_from_path(DATA_ZIP_FILE)
    else:
        st.error(
            "A base não foi encontrada. Coloque o arquivo 'base_queimadas_INPE_2020_2025_didatica_limpa.csv' "
            "ou 'base_queimadas_INPE_2020_2025_didatica_limpa.zip' na mesma pasta do app.py, "
            "ou carregue um CSV/ZIP pela barra lateral."
        )
        st.stop()
