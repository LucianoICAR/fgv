...
demandas = st.sidebar.slider("Quantidade de demandas no sistema", 5, 60, 24)
...
with aba2:
    ...
    kanban_cols = [
        ("Backlog", demandas, None),
        ("Análise", min(wip_analise, demandas), wip_analise),
        ("Desenvolvimento", min(wip_dev, demandas), wip_dev),
        ("Testes", min(wip_teste, demandas), wip_teste),
        ("Concluído", 0, None)
    ]

    for col, (titulo, cards, wip) in zip([col1, col2, col3, col4, col5], kanban_cols):
        with col:
            if wip:
                st.subheader(f"{titulo}")
                st.caption(f"WIP: {wip}")
            else:
                st.subheader(titulo)

            max_cards = min(cards, 8)
            for i in range(max_cards):
                st.info(f"História #{i+1}")

            if cards > 8:
                st.caption(f"+ {cards - 8} demandas")

    st.warning("O quadro acima é uma representação didática. O objetivo é discutir o efeito do WIP no fluxo, não simular cada cartão individualmente.")
...
- Status em relação à meta de **{meta_ciclo} dias**: **{status_tempo_ciclo(resultado['tempo_ciclo_estimado'])}**.
""")
...
with aba4:
    ...
    for nome, cfg in cenarios.items():
        ...
        linhas.append({
            ...
            "Status": status_tempo_ciclo(r["tempo_ciclo_estimado"])
        })
...
""")
    ...
    st.download_button(
        ...
        data="""ATIVIDADE — KANBAN, GARGALOS E WIP
...
6. Conclusão
""",
        file_name="atividade_kanban_wip.txt",
        mime="text/plain"
    )

st.divider()
st.caption("Modelo didático para ensino de Kanban, gargalos, WIP, throughput e tempo de ciclo.")
