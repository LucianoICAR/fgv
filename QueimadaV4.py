...
def read_csv_optimized(file_or_path) -> pd.DataFrame:
    ...
    return pd.read_csv(file_or_path, dtype=CSV_DTYPE_HINTS, low_memory=False)
...
def load_data_from_upload(uploaded_file) -> pd.DataFrame:
    df = read_csv_optimized(uploaded_file)
    return _normalize_columns(df)
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "Carregar outro CSV, se desejar",
        type=["csv"],
        help=(
            "O sistema já procura a base limpa no mesmo diretório do app. "
            "Para aceitar arquivos de até 500 MB, mantenha o arquivo .streamlit/config.toml incluído no projeto."
        ),
    )
    st.caption("Limite configurado para upload: até 500 MB.")
