...
def train_model(
    ...
    labels = [label for label in RISK_ORDER if label in sorted(y_test.unique().tolist())]
    ...
    if hasattr(model_step, "feature_importances_"):
        ...
        fi = pd.DataFrame({"atributo_codificado": encoded_feature_names, "importancia": importances})
        ...
        fi["atributo"] = fi["atributo_codificado"].apply(original_feature)
        feature_importance = (
            fi.groupby("atributo", as_index=False)["importancia"].sum()
    ...
    return pipeline, metadata, accuracy, cm, report, feature_importance, X_test, y_test, y_pred
        ...
        if st.button("Treinar modelo", type="primary"):
            with st.spinner("Treinando modelo de IA..."):
                try:
                    ...
                    (
                        pipeline, metadata, accuracy, cm, report, feature_importance,
                        X_test, y_test, y_pred,
                    ) = results
                    st.session_state["modelo_queimadas"] = pipeline
                    st.session_state["metadata_modelo"] = metadata
                    st.session_state["avaliacao_modelo"] = {
                        "accuracy": accuracy,
                        "cm": cm,
                        "report": report,
                    ...
                    st.success("Modelo treinado com sucesso. Veja a avaliação na próxima aba.")
                    st.metric("Acurácia no conjunto de teste", f"{accuracy:.1%}")
                    st.write("Metadados do treinamento:")
                    st.json(metadata)
                except Exception as exc:
...
with tab4:
    ...
    if "modelo_queimadas" not in st.session_state:
        st.info("Treine um modelo na aba **Treinar a IA** para visualizar a avaliação.")
    else:
        metadata = st.session_state["metadata_modelo"]
        evaluation = st.session_state["avaliacao_modelo"]
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Modelo", metadata["model_name"])
        c2.metric("Amostra usada", f"{metadata['sample_size_used']:,}".replace(",", "."))
        c3.metric("Teste", f"{metadata['test_size']:,}".replace(",", "."))
        c4.metric("Acurácia", f"{accuracy:.1%}")

        st.subheader("Matriz de confusão")
        st.write(
            "A matriz mostra onde a IA acertou e onde confundiu as classes. A diagonal principal representa os acertos."
        )
        cm_df = pd.DataFrame(cm, index=labels, columns=labels)
        fig_cm = px.imshow(
            cm_df,
            text_auto=True,
            labels=dict(x="Classe prevista", y="Classe real", color="Registros"),
            x=labels,
            y=labels,
        ...
        st.subheader("Precisão, recall e F1-score")
        rows = []
        for label in labels:
            rows.append({
                "Classe": label,
                "Precisão": report[label]["precision"],
                "Recall": report[label]["recall"],
                "F1-score": report[label]["f1-score"],
                "Registros no teste": int(report[label]["support"]),
            })
        report_df = pd.DataFrame(rows)
        st.dataframe(
            report_df.style.format({"Precisão": "{:.1%}", "Recall": "{:.1%}", "F1-score": "{:.1%}"}),
            use_container_width=True,
            hide_index=True,
        )

        st.info(
            "Para prevenção ambiental, o recall da classe **Alto** é muito importante: ele indica quantos casos de risco alto "
            "o modelo conseguiu identificar entre todos os casos que eram realmente alto."
        )
        ...
        if feature_importance is not None and not feature_importance.empty:
            ...
            fig_fi = px.bar(
                ...
                title="Quais fatores a IA mais usou para classificar o risco",
            )
