...
with tab3:
    ...
    t1.metric("Registros disponíveis para treino", f"{available_training_records:,}".replace(",", "."))
    t2.metric("Classe Alto", f"{int(class_distribution.loc[class_distribution['Classe'] == 'Alto', 'Registros disponíveis'].iloc[0]):,}".replace(",", "."))
    t3.metric("Classes disponíveis", int((class_distribution["Registros disponíveis"] > 0).sum()))
    ...
    with c2:
        if available_training_records >= 1000:
            max_records = min(1_000_000, available_training_records)
            default_records = min(max_records, max(100_000, int(available_training_records * 0.30)))
            step_records = 10_000 if max_records >= 100_000 else 1_000
            min_records = min(1_000, max_records)
            sample_size = st.slider(
                "Registros para treino",
                min_value=min_records,
                max_value=max_records,
                value=default_records,
                step=step_records,
                help=(
                    "O limite e o valor inicial são calculados a partir dos registros válidos da base carregada. "
                    "Bases maiores permitem amostras maiores; valores muito altos podem aumentar o tempo de treino."
                ),
            )
        else:
    ...
    st.caption(
        ...
        "e manter o sistema mais leve para a apresentação."
    )
        ...
        if st.button("Treinar modelo", type="primary"):
            with st.spinner("Treinando modelo de IA..."):
                try:
                    ...
                    a2.metric("Registros usados", f"{metadata['sample_size_used']:,}".replace(",", "."))
                    a3.metric("Registros de teste", f"{metadata['test_size']:,}".replace(",", "."))
                    st.caption(
                        "A acurácia é calculada em uma parte separada da base, reservada apenas para teste. "
                        "Ela indica a proporção geral de classificações corretas."
                    )
                except Exception as exc:
